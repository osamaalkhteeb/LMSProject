  ...
  useEffect(() => {
    const fetchStats = async () => {
      try {
        ...
        const [userStatsResponse, courseStatsResponse, pendingCoursesResponse] = await Promise.all([
          getUserStats(),
          getCourseStats(),
          getPendingCourses({ limit: 1000 }) // Get all pending courses to count them
        ]);
        
        setStats({
          userStats: userStatsResponse.data,
          courseStats: courseStatsResponse.data,
              ...
              <Grid item xs={6} md={3}>
                <Box textAlign="center">
                  <Typography variant="h4" fontWeight="bold" color="secondary.main">
                    {stats.courseStats?.published_courses || 0}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    Published Courses
                  </Typography>
              ...
              <Grid item xs={6} md={3}>
                <Box textAlign="center">
                  <Typography variant="h4" fontWeight="bold" color="warning.main">
                    {stats.userStats?.recent_logins || 0}
                  </Typography>
                  <Typography variant="body2" color="text.secondary">
                    Recent Logins
                  </Typography>
